        ...
        ref.getDocuments(completion: {snapshot, error in
            ...
            if let snapshot = snapshot{
                for document in snapshot.documents{
                    ...
                    let order = ModelRow(id: identefire, from: from, to: to, weidth: weight, description: description, accountEmail: accaunt, rate: rate)
...
//MARK: - создать новый заказ
    func addNewDataOrder(modelOrder: ModelRow) {
        let db = Firestore.firestore()
              do {
                  _ = try db.collection("order").document(modelOrder.id).setData(["from": modelOrder.from,"to": modelOrder.to, "weight": modelOrder.weidth,"description":modelOrder.description,"accauntEmail":modelOrder.accountEmail,"rate": modelOrder.rate])
                  fetchDataOrder()
              }
              catch {
